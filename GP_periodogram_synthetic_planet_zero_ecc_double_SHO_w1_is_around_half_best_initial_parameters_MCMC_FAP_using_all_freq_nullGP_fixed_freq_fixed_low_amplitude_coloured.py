...
def process_FAP_marginal(w0_list, y, t, yerr, timespan_obs, weighted_mean_value, null_log_like, df):
    # Initial parameter guesses
    w0copy = w0copy = w0 = 0.6 #random.choice(w0_list[2:-3])         w0 = random.choice(w0_list[2:-3])#
    print('w0list =',w0_list)
    print('w0copy and w0 choosen randomly = ',w0copy, w0)
    if 'w0' not in df.columns:
        raise ValueError("CSV file must contain 'w0'")
    closest_w0_idx = (df['w0'] - w0).abs().idxmin()
    filtered_df = df[df['w0'] == w0]
    ...
    #bounds = [(-15,15),(np.log(0.5), np.log(1000)), (-15, 15),(-15,15),(np.log(0.5), np.log(1000)), (-15, 15)] # Adjusting the order based on params
    # Define parameter bounds
    log_w0_min=np.log(w0_list[0]) #np.log(2 * w0*0.95)
    log_w0_max=np.log(w0_list[-1]) #np.log(2 * w0*1.05)
    #log_Q1 = max(log_Q1, np.log(0.51))
    #log_Q2 = max(log_Q2, np.log(0.51))
    bounds = {"log_S0": (-15, 15), "log_Q": (log_Q_lowest, 15), "log_omega0": (log_w0_min, log_w0_max)}
    # Define two-term kernel: one for w0, one for w1 = 2*w0
    
    term1 = terms.SHOTerm(log_S0=log_S0_1, log_Q=log_Q1, log_omega0=np.log(w0), bounds=bounds)
   
    bounds2 = {"log_S0": (-15, 15), "log_Q": (log_Q_lowest, 15), "log_omega0": (log_w0_min, log_w0_max+np.log(2))}
    term2 = terms.SHOTerm(log_S0=log_S0_2, log_Q=log_Q2, log_omega0=np.log(2 * w0), bounds=bounds2)
    ...
    # Define negative log-likelihood function with linked omega0
    def neg_log_like(params, y, gp):
        # params: [log_S0_1, log_Q1, log_omega0_1, log_S0_2, log_Q2, log_sigma]
        log_omega0_1 = params[2]
        log_omega0_2 = log_omega0_1 + np.log(2)
        full_params = [params[0], params[1], log_omega0_1, params[3], params[4], log_omega0_2, params[5]]
        gp.set_parameter_vector(full_params)
        return -gp.log_likelihood(y)
   
    # Get initial parameters and set bounds
    initial_full = gp.get_parameter_vector()
    initial_reduced = [initial_full[0], initial_full[1], initial_full[2], initial_full[3], initial_full[4], initial_full[6]]
    
    full_bounds = gp.get_parameter_bounds()
    # Adjust the bound for shared log_omega0_1 to ensure log_omega0_2 stays within its bounds
    log_w0_max_adjusted = log_w0_max - np.log(2)
    reduced_bounds = [
        full_bounds[0],  # log_S0_1
        full_bounds[1],  # log_Q1
        (log_w0_min, log_w0_max_adjusted),  # log_omega0_1
        full_bounds[3],  # log_S0_2
        full_bounds[4],  # log_Q2
        full_bounds[6]   # log_sigma
    ]
   
    # Optimize parameters
    r = minimize(neg_log_like, initial_reduced, method="L-BFGS-B", bounds=reduced_bounds, args=(y, gp))
    
    # Set the optimized parameters back to gp
    optimized_log_omega0_1 = r.x[2]
    optimized_log_omega0_2 = optimized_log_omega0_1 + np.log(2)
    optimized_full = [r.x[0], r.x[1], optimized_log_omega0_1, r.x[3], r.x[4], optimized_log_omega0_2, r.x[5]]
    gp.set_parameter_vector(optimized_full)
    ...
    S0_2 = np.exp(optimized_params['kernel:terms[1]:log_S0'])
    Q_2 = np.exp(optimized_params['kernel:terms[1]:log_Q'])
    w0 = np.exp(optimized_params['kernel:terms[0]:log_omega0'])
    w_2 = np.exp(optimized_params['kernel:terms[1]:log_omega0']) #2*w0#
    jitter=np.exp(optimized_params['kernel:terms[2]:log_sigma'])
    #you should not keep w_2 free to optimize beyound 5% of 2*w0 (0.5 time the period). Because then it prioretizes optimiing double the period of rotation of star.
    delta_log_like = gp.log_likelihood(y) - null_log_like
...
n_sim = 100
max_dlnL_sim = np.zeros(n_sim)
highest_delta_log_likelihood_solution_list=[]
for k in range(n_sim):
    print(k)
    try:
        w0=highest_delta_log_likelihood_solution[5]
    except Exception as e:
        print(f"Error: {e}")
        # Optionally, return a default value or continue with other actions
        continue
    ...
    #y=y-weighted_mean_value
    #weighted_mean_value=0
    #Std = sqrt (uncertainityˆ2 + jitterˆ2)
    ########################old############null_log_like = np.sum(norm.logpdf(y, loc=weighted_mean_value, scale=yerr))
    try:
        result_FAP=process_FAP_marginal(w0_list, y, t, yerr, timespan_obs, weighted_mean_value, null_log_like,df_copy)
        #result_FAP = Parallel(n_jobs=-1)(delayed(process_w0)(w0, y, t, yerr, timespan_obs, weighted_mean_value, null_log_like,df_copy) for w0 in tqdm(w0_list, desc="Processing FAP"))   # Replace this with your actual line of code
        #return result
        print(result_FAP)
    except Exception as e:
        print(f"Error: {e}")
        # Optionally, return a default value or continue with other actions
        result_FAP= None  # For example, return None if it fails
    #result = Parallel(n_jobs=-1)(delayed(process_w0)(w0, y, t, yerr, timespan_obs, weighted_mean_value, null_log_like,df) for w0 in w0_list)
    if result_FAP is not None and len(result_FAP) > 0:
        highest_delta_log_likelihood_solution = result_FAP
        #highest_delta_log_likelihood_solution = max(result_FAP, key=lambda item: item[4])
        # Continue with your processing using highest_delta_log_likelihood_solution
    else:
        print("Computation failed or returned no results, skipping processing.")
        highest_delta_log_likelihood_solution = None  # Or set a default value
    #highest_delta_log_likelihood_solution = max(result, key=lambda item: item[4])
    highest_delta_log_likelihood_solution_list.append(highest_delta_log_likelihood_solution)
...
def gridsearch_parameters_FAP(t,y,yerr,w0_list, weighted_mean_value, null_log_like):
    # Grid bounds
    log_S0_bounds = (-10, 5)
    log_Q_lowest = np.log(0.502)
    log_Q_bounds = (log_Q_lowest, 10)
...
for k in range(n_sim):
    ...
    # You can also compute the final weighted mean using the optimal jitter
    optimal_weights = 1.0 / (yerr**2 + jitter**2)
    weighted_mean_value = np.average(y, weights=optimal_weights)
    print(f"Weighted Mean (with optimal jitter): {weighted_mean_with_jitter:.4f}")
    ...
    #Std = sqrt (uncertainityˆ2 + jitterˆ2)
    ########################old############null_log_like = np.sum(norm.logpdf(y, loc=weighted_mean_value, scale=yerr))
    optimal_df = gridsearch_parameters_FAP(t,y,yerr,w0_list,weighted_mean_value,null_log_like)
     

    try:
        result_FAP = Parallel(n_jobs=-1)(delayed(process_w0)(w0, y, t, yerr, timespan_obs, weighted_mean_value, null_log_like,optimal_df) for w0 in tqdm(w0_list, desc="Processing grid search"))   # Replace this with your actual line of code
        #result_FAP = Parallel(n_jobs=-1)(delayed(process_w0)(w0, y, t, yerr, timespan_obs, weighted_mean_value, null_log_like,df_copy) for w0 in tqdm(w0_list, desc="Processing FAP"))   # Replace this with your actual line of code
        #return result
        S0_values = [i[0] for i in result_FAP]
        S2_values = [i[1] for i in result_FAP]
        w0_values = [i[5] for i in result_FAP]
        w2_values = [i[6] for i in result_FAP]
        Q0_values = [i[2] for i in result_FAP]
        Q2_values = [i[3] for i in result_FAP]
        delta_log_like = [i[4] for i in result_FAP]
        lifetime0_values =  np.array(Q0_values)/np.array(w0_values)*2*np.pi#2*np.pi/w0_values*Q0_values
        lifetime2_values = np.array(Q2_values)/np.array(w0_values)*2*np.pi
        # Frequency calculations
        Frequency_val = np.array(w0_values) / (2 * np.pi)
        plt.figure(figsize =(10,6), dpi=300 )
        plt.plot(Frequency_val,delta_log_like,'r')
        plt.xlabel('Period of first oscillator [1/d]')
        plt.ylabel('Delta log likelihood')
        #plt.title('1D GP Periodogram '+name)
        ...
        #plt.minorticks_on()  # This enables minor ticks
        #plt.grid(which='minor', color='#DDDDDD', linestyle='--', alpha=0.6)
        plt.savefig(str(k)+"_GP 1D double sho "+name+" after gridsearch on initial parameters.png")
        plt.show(block=False)
        plt.close()
        ...
        #plt.ylim([0,20])
        plt.legend()
        plt.savefig(str(k)+"_GLS "+name+" delta log likelihood peridogram.png")
        plt.show(block=False)
        plt.close()
...
flat_list = [item for sub in highest_delta_log_likelihood_solution_list if sub is not None for item in sub]
df_heighest_delta_ll_solutions = pd.DataFrame(flat_list, columns=['S0_1', 'S0_2', 'Q_1', 'Q_2', 'delta_log_like','w0','w_2','jitter'])


#plt.figure(figsize=(16,11))  
#flat_list = [item for sub in highest_delta_log_likelihood_solution_list if sub is not None for item in sub]
#df_heighest_delta_ll_solutions = pd.DataFrame(highest_delta_log_likelihood_solution_list, columns=['S0_1', 'S0_2', 'Q_1', 'Q_2', 'delta_log_like','w0','w_2', 'jitter'])


df_sorted = df_heighest_delta_ll_solutions.sort_values(by=df_heighest_delta_ll_solutions.columns[4], ascending=True)
output_file = 'highest_delta_log_likelihood_solution_list_1D_GP_single_frequency_'+name+'.csv'
df_sorted.to_csv(output_file, index=False)

end_time = time.perf_counter()
print("execution time = ", end_time - start_time)

#ΔlnL thresholds: FAP=0.01 → 102.57174,  FAP=0.001 → 115.35600,  FAP=0.1 → 80.71130
print(np.median(dt))
