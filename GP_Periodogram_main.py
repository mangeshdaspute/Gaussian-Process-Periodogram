...
NAME = "activity RV asymmetric two spot region configuration gridsearch 5"
...
RUN_GRIDSEARCH = True  # set False after the first run to save time
...
def main() -> None:
    ...
    w0min = 2*np.pi / (timespan_obs*OVERSAMPLING_FACTOR)
    ...
    if RUN_GRIDSEARCH:
        ...
        df_initial = gridsearch_initial_params(
            ...
            NAME, n_points=GRIDSEARCH_N_POINTS,
        )
    else:
        print(f"[7/9] Loading gridsearch results from '{GRIDSEARCH_CSV}'")
        df_initial = pd.read_csv(GRIDSEARCH_CSV)
        print(f"  Loaded {len(df_initial)} rows")
